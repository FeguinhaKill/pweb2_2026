@extends('main')
@section('titulo', 'Formulário Acessórios')
@section('conteudo')
    ...
    <div class="col-12">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-primary text-white">
                <h3 class="mb-0">{{ !empty($dado->id) ? 'Editar Acessório' : 'Novo Acessório' }}</h3>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                @php
                    if (!empty($dado->id)) {
                        $action = route('acessorios.atualizar', $dado->id);
                    } else {
                        $action = route('acessorios.salvar');
                    }
                @endphp
                ...
                <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mt-4">
                        <div class="col d-flex gap-2 flex-wrap">
                            ...
                            <a href="{{ route('acessorios.index') }}" class="btn btn-secondary">Voltar à Lista</a>
                        </div>
                    </div>
                </form>
            </div>
